...
class FNOInverseClassifier(nn.Module):
    def __init__(self, modes1, modes2, width):
        ...
        self.fc2 = nn.Linear(128, 1)  # 出力チャネル1（2値分類）

        self.sigmoid = nn.Sigmoid()  # 出力層に明示的なシグモイド

    def forward(self, x):
        grid = self.get_grid(x.shape, x.device)
        x = torch.cat((x, grid), dim=-1)  # (B, H, W, 1+2)
        x = self.fc0(x)
        x = x.permute(0, 3, 1, 2)
        ...
        x = x3.permute(0, 2, 3, 1)
        x = self.fc1(x)
        x = torch.relu(x)
        x = self.fc2(x)
        x = self.sigmoid(x)  # 確率マップ（0~1）
        return x  # (B, H, W, 1)
