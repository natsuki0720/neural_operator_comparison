...
class CNO_heavy(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, hidden_channels=64):
        super().__init__()
        self.encoder1 = nn.Sequential(
            nn.Conv2d(in_channels + 2, hidden_channels, 3, padding=1), nn.ReLU(),
            nn.Conv2d(hidden_channels, hidden_channels, 3, padding=1), nn.ReLU(),
        )
        self.pool1 = nn.MaxPool2d(2)

        self.encoder2 = nn.Sequential(
            nn.Conv2d(hidden_channels, hidden_channels * 2, 3, padding=1), nn.ReLU(),
            nn.Conv2d(hidden_channels * 2, hidden_channels * 2, 3, padding=1), nn.ReLU(),
        )
        self.pool2 = nn.MaxPool2d(2)

        self.bottleneck = nn.Sequential(
            nn.Conv2d(hidden_channels * 2, hidden_channels * 4, 3, padding=1), nn.ReLU(),
            nn.Conv2d(hidden_channels * 4, hidden_channels * 4, 3, padding=1), nn.ReLU(),
        )

        self.up1 = nn.Upsample(scale_factor=2, mode='nearest')
        self.decoder1 = nn.Sequential(
            nn.Conv2d(hidden_channels * 4, hidden_channels * 2, 3, padding=1), nn.ReLU(),
            nn.Conv2d(hidden_channels * 2, hidden_channels * 2, 3, padding=1), nn.ReLU(),
        )

        self.up2 = nn.Upsample(scale_factor=2, mode='nearest')
        self.decoder2 = nn.Sequential(
            nn.Conv2d(hidden_channels * 2, hidden_channels, 3, padding=1), nn.ReLU(),
            nn.Conv2d(hidden_channels, out_channels, 3, padding=1),
        )
